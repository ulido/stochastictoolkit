...
class WorldBoundaries(BoundaryCondition):
    ...
    def _B_reflecting_boundary(self, positions):
        ...
        to_update = (diff**2).sum(axis=1) >= 1

        for i, bbox in enumerate(self.__boundary_bounds):
            inside_bbox = ((positions[:, 0] > bbox[0, 0]) &
                           (positions[:, 0] < bbox[0, 1]) &
                           (positions[:, 1] > bbox[1, 0]) &
                           (positions[:, 1] < bbox[1, 1]))
            if inside_bbox.any():
...
class World:
    def __init__(self, filename):
        ...
        # Split boundary shape into multiple Polygons for performance.
        ...
        saturation_thresholdB = 3
        memory_decay_rateC = 0.2
        saturation_thresholdC = 4
        memory_decay_rateD = 0.5
        saturation_thresholdD = 5
        ...
        def velocity(receptor_collections):
            ...
            vA = np.linalg.norm(dirA)
            vB = np.linalg.norm(dirB)
            vC = np.linalg.norm(dirC)
            vD = np.linalg.norm(dirD)
            vl = (vA+2*vB+3*vC+4*vD)
            #if vt < 1e-8:
            #    return np.zeros((2,))
